...
template <typename Function, typename Signature> struct CppClosure {};

// END ******** CppClosure prototype ********
// BEGIN ******** invoker ********

template <typename Function, typename R> struct invoker {
  static inline void invoke(const Function &f, Value &result) {
    ValueImpl<R>::set(result, f());
  }
};

template <typename Function> struct invoker<Function, void> {
...
template <typename T, typename R, typename... Args> class MemberFunction {
public:
...
public:
  ...
  inline R operator()(Args &&...args) const {
    return m_function(std::forward<Arg1>(m_firstArg),
...
inline BoundArgumentFunction<F, R, Arg1, Args...> partial_bind(F &&f,
                                                               Arg1 &&a1) {
  return BoundArgumentFunction<F, R, Arg1, Args...>(std::forward<F>(f),
                                                    std::forward<Arg1>(a1));
}
...
inline void unpackAndInvoke(F &&function, Value &result,
  ...
  typedef typename boost::remove_const<
      typename boost::remove_reference<Arg1>::type>::type CleanArg1;
  typedef BoundArgumentFunction<F, R, Arg1, Args...> F1;

  CleanArg1 &&boundArg = ValueImpl<CleanArg1>::get(*argsBegin);
  F1 &&f = partial_bind<F, R, Arg1, Args...>(std::forward<F>(function),
                                             std::forward<Arg1>(boundArg));

  unpackAndInvoke<F1, R, Args...>(
      std::forward<F1>(f), result,
...
bool connect(void *instance, const char *detailedSignal, T *receiver,
  ...
  return Private::connect(instance, detailedSignal, Quark(), receiver,
                          Private::GetDestroyNotifier<T>(),
...
#define BOOST_PP_ITERATION_PARAMS_1                                            \
  (3, (0, QGLIB_CONNECT_MAX_ARGS, "QGlib/connectimpl.h"))
...
/*
    This part is included from BOOST_PP_ITERATE(). It defines a CppClosureN
   class (where N is the number of template arguments it takes) and a
   specialization for class CppClosure, so that the CppClosure<R (Args...), F>
   syntax is supported. This part is included multiple times
   (QGLIB_CONNECT_MAX_ARGS defines how many), and each time it defines those
   classes with different number of arguments. The concept is based on the
   implementation of boost::function.
*/

#define QGLIB_CONNECT_IMPL_NUM_ARGS BOOST_PP_ITERATION()
...
#define QGLIB_CONNECT_IMPL_UNPACK_ARGS_STEP(z, n, list)                        \
  , ValueImpl<typename boost::remove_const<                                    \
        typename boost::remove_reference<A##n>::type>::type>::get(list.at(n))

#define QGLIB_CONNECT_IMPL_UNPACK_ARGS(list)                                   \
  BOOST_PP_REPEAT(QGLIB_CONNECT_IMPL_NUM_ARGS,                                 \
                  QGLIB_CONNECT_IMPL_UNPACK_ARGS_STEP, list)
  ...
  public:
    ...
    virtual void marshaller(Value &result, const QList<Value> &params) {
      ...
      boost::function<R()> callback =
          boost::bind<R>(m_function QGLIB_CONNECT_IMPL_UNPACK_ARGS(params));
...
// partial specialization of struct CppClosure to support the CppClosure<F, R
// (Args...)> syntax
template <typename F, typename R QGLIB_CONNECT_IMPL_TRAILING_TEMPLATE_PARAMS>
struct CppClosure<F, R(QGLIB_CONNECT_IMPL_TEMPLATE_ARGS)>
    : public QGLIB_CONNECT_IMPL_CPPCLOSUREN<
          F, R QGLIB_CONNECT_IMPL_TRAILING_TEMPLATE_ARGS> {};
...
bool connect(void *instance, const char *detailedSignal, T *receiver,
  ...
  Private::ClosureDataBase *closure = Private::CppClosure<
      ...
      R(QGLIB_CONNECT_IMPL_TEMPLATE_ARGS)>::create(f, flags & PassSender);
