...
TagList::Data::Data() : QSharedData() { taglist = gst_tag_list_new_empty(); }

TagList::Data::Data(const GstTagList *tl) : QSharedData() {
...
TagList::Data::Data(const TagList::Data &other) : QSharedData(other) {
  taglist = gst_tag_list_copy(other.taglist);
}

TagList::Data::~Data() { gst_tag_list_unref(taglist); }

#endif // DOXYGEN_RUN

TagList::TagList() : d(new Data) {}

TagList::TagList(const GstTagList *taglist) : d(new Data(taglist)) {}

TagList::~TagList() {}

TagList::TagList(const TagList &other) : d(other.d) {}
...
bool TagList::isEmpty() const { return gst_tag_list_is_empty(d->taglist); }

void TagList::insert(const TagList &other, TagMergeMode mode) {
  gst_tag_list_insert(d->taglist, other.d->taglist,
                      static_cast<GstTagMergeMode>(mode));
}

// static
TagList TagList::merge(const TagList &firstList, const TagList &secondList,
                       TagMergeMode mode) {
  GstTagList *taglist = gst_tag_list_merge(firstList, secondList,
                                           static_cast<GstTagMergeMode>(mode));

  // avoid copying the merged taglist by freeing the new one and assigning this
  // one to d->taglist
  TagList tl;
...
void TagList::setTagValue(const char *tag, const QGlib::Value &value,
                          TagMergeMode mode) {
  gst_tag_list_add_value(d->taglist, static_cast<GstTagMergeMode>(mode), tag,
                         value);
}
...
TagList::operator GstTagList *() { return d->taglist; }

TagList::operator const GstTagList *() const { return d->taglist; }
...
void TagList::setDate(const QDate &value) {
  ...
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_DATE, date,
                   NULL);
}
...
void TagList::setTrackNumber(quint32 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_TRACK_NUMBER,
                   value, NULL);
}
...
void TagList::setTrackCount(quint32 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_TRACK_COUNT,
                   value, NULL);
}
...
void TagList::setAlbumVolumeNumber(quint32 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL,
                   GST_TAG_ALBUM_VOLUME_NUMBER, value, NULL);
}
...
void TagList::setAlbumVolumeCount(quint32 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL,
                   GST_TAG_ALBUM_VOLUME_COUNT, value, NULL);
}
...
void TagList::setVersion(const QString &value) {
  gst_tag_list_add_value(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_VERSION,
                         QGlib::Value::create(value));
}
...
void TagList::setIsrc(const QString &value) {
  gst_tag_list_add_value(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_ISRC,
                         QGlib::Value::create(value));
}
...
void TagList::setLicense(const QString &value) {
  gst_tag_list_add_value(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_LICENSE,
                         QGlib::Value::create(value));
}
...
void TagList::setDuration(quint64 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_DURATION,
                   value, NULL);
}
...
void TagList::setCodec(const QString &value) {
  gst_tag_list_add_value(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_CODEC,
                         QGlib::Value::create(value));
}
...
void TagList::setBitrate(quint32 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_BITRATE,
                   value, NULL);
}
...
void TagList::setNominalBitrate(quint32 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL,
                   GST_TAG_NOMINAL_BITRATE, value, NULL);
}
...
void TagList::setMinimumBitrate(quint32 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL,
                   GST_TAG_MINIMUM_BITRATE, value, NULL);
}
...
void TagList::setMaximumBitrate(quint32 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL,
                   GST_TAG_MAXIMUM_BITRATE, value, NULL);
}
...
void TagList::setSerial(quint32 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_SERIAL, value,
                   NULL);
}
...
void TagList::setEncoder(const QString &value) {
  gst_tag_list_add_value(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_ENCODER,
                         QGlib::Value::create(value));
}
...
void TagList::setEncoderVersion(quint32 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL,
                   GST_TAG_ENCODER_VERSION, value, NULL);
}
...
void TagList::setTrackGain(double value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_TRACK_GAIN,
                   value, NULL);
}
...
void TagList::setTrackPeak(double value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_TRACK_PEAK,
                   value, NULL);
}
...
void TagList::setAlbumGain(double value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_ALBUM_GAIN,
                   value, NULL);
}
...
void TagList::setAlbumPeak(double value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_ALBUM_PEAK,
                   value, NULL);
}
...
void TagList::setReferenceLevel(double value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL,
                   GST_TAG_REFERENCE_LEVEL, value, NULL);
}
...
void TagList::setBeatsPerMinute(double value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL,
                   GST_TAG_BEATS_PER_MINUTE, value, NULL);
}
...
void TagList::setShowEpisodeNumber(quint32 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL,
                   GST_TAG_SHOW_EPISODE_NUMBER, value, NULL);
}
...
void TagList::setShowSeasonNumber(quint32 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL,
                   GST_TAG_SHOW_SEASON_NUMBER, value, NULL);
}
...
void TagList::setUserRating(quint32 value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL, GST_TAG_USER_RATING,
                   value, NULL);
}
...
void TagList::setGeoLocationHorizontalError(double value) {
  gst_tag_list_add(d->taglist, GST_TAG_MERGE_REPLACE_ALL,
                   GST_TAG_GEO_LOCATION_HORIZONTAL_ERROR, value, NULL);
}
...
QDebug operator<<(QDebug debug, const TagList &taglist) {
  debug.nospace() << "QGst::TagList("
                  << QGlib::Private::stringFromGCharPtr(
