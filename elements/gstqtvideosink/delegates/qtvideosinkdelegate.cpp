/*
   ...
   <qt-info@nokia.com> Copyright (C) 2011 Collabora Ltd. <info@collabora.com>
...
QtVideoSinkDelegate::QtVideoSinkDelegate(GstElement *sink, QObject *parent)
    : BaseDelegate(sink, parent), m_painter(0), m_supportedPainters(Generic)
#ifndef GST_QT_VIDEO_SINK_NO_OPENGL
      ,
      m_glContext(0)
#endif
...
QtVideoSinkDelegate::~QtVideoSinkDelegate() { destroyPainter(); }
...
void QtVideoSinkDelegate::paint(QPainter *painter, const QRectF &targetArea) {
  ...
  if (m_glContext) {
    Q_ASSERT_X(m_glContext == QGLContext::currentContext(),
               ...
               "GL surface that has the same context as the one given on the "
               "glcontext property");
  }
#endif
  ...
  if (!m_buffer) {
    ...
    if (targetArea != m_areas.targetArea || m_formatDirty ||
      ...
      Qt::AspectRatioMode aspectRatioMode =
          m_forceAspectRatio ? Qt::KeepAspectRatio : Qt::IgnoreAspectRatio;
    ...
    // if either pixelFormat or frameSize have changed, we need to reset the
    // painter and/or change painter, in case the current one does not handle
...
QGLContext *QtVideoSinkDelegate::glContext() const { return m_glContext; }
...
void QtVideoSinkDelegate::setGLContext(QGLContext *context) {
  ...
  GST_LOG_OBJECT(m_sink, "Done setting GL context. m_supportedPainters=%x",
                 (int)m_supportedPainters);
}
...
void QtVideoSinkDelegate::changePainter(const BufferFormat &format) {
  ...
  if (GenericSurfacePainter::supportedPixelFormats().contains(
          format.videoFormat())) {
    possiblePainters.push(Generic);
  }

#ifndef GST_QT_VIDEO_SINK_NO_OPENGL
  if (OpenGLSurfacePainter::supportedPixelFormats().contains(
          format.videoFormat())) {
  ...
  while (!possiblePainters.isEmpty()) {
    ...
    try {
      ...
      GST_ELEMENT_WARNING(m_sink, RESOURCE, FAILED, ("Failed to start painter"),
                          ("%s", error.toUtf8().constData()));
      delete m_painter;
      m_painter = 0;
    }
  }
